...
def validate_app_id(app_id: str) -> str:
    ...
    if len(app_id_str) < 9 or len(app_id_str) > 10:
        raise ValidationError(f"App ID should be 9-10 digits, got {len(app_id_str)} digits: {app_id_str}")
...
def validate_vendor_number(vendor_number: str) -> str:
    ...
    if len(vendor_str) < 8 or len(vendor_str) > 9:
        raise ValidationError(f"Vendor number should be 8-9 digits, got {len(vendor_str)} digits: {vendor_str}")
...
def validate_locale(locale: str) -> str:
    ...
    Args:
        locale: The locale to validate (e.g., 'en-US', 'fr-FR')
    ...
    # Basic locale format: language-country (e.g., en-US, fr-FR)
    ...
    if isinstance(date_input, datetime):
        ...
        try:
            ...
            try:
                ...
                raise ValidationError(f"Invalid date format. Expected 'YYYY-MM-DD' or 'MM/DD/YYYY', got: {date_input}")
    else:
        raise ValidationError(f"Invalid date type. Expected str, date, or datetime, got: {type(date_input)}")
...
def validate_report_frequency(frequency: str) -> str:
    ...
    if frequency not in valid_frequencies:
        raise ValidationError(f"Invalid frequency. Must be one of: {valid_frequencies}, got: {frequency}")
...
def validate_report_type(report_type: str) -> str:
    ...
    """
    valid_types = ['SALES', 'SUBSCRIPTION', 'SUBSCRIPTION_EVENT', 'SUBSCRIBER', 'FINANCIAL']
    ...
    if report_type not in valid_types:
        raise ValidationError(f"Invalid report type. Must be one of: {valid_types}, got: {report_type}")
...
def validate_report_subtype(report_subtype: str) -> str:
    ...
    if report_subtype not in valid_subtypes:
        raise ValidationError(f"Invalid report subtype. Must be one of: {valid_subtypes}, got: {report_subtype}")
