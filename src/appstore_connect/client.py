...
class AppStoreConnectAPI:
    ...
    @sleep_and_retry
    @limits(calls=50, period=3600)  # Apple's rate limit: 50 requests per hour
    def _make_request(
        self, 
        method: str = "GET", 
        url: str = None, 
        ...
        # Handle both direct URL and endpoint patterns for backward compatibility
            ...
            if weekly_start <= weekly_end:
                ...
                while current_sunday <= weekly_end:
                    try:
                        ...
                        logging.warning(f"Error fetching weekly data for week of {current_sunday}: {e}")
    ...
    def get_app_info_localizations(self, app_info_id: str) -> Optional[Dict]:
        ...
        response = self._make_request(
            method="GET", 
            endpoint=f"/appInfos/{app_info_id}/appInfoLocalizations"
        )
        if response.status_code == 200:
            return response.json()
        return None
    ...
    def update_app_name(self, app_id: str, name: str, locale: str = "en-US") -> bool:
        ...
        if len(name) > 30:
            raise ValidationError(f"App name too long ({len(name)} chars). Maximum is 30 characters.")
    ...
    def update_app_subtitle(self, app_id: str, subtitle: str, locale: str = "en-US") -> bool:
        ...
        if len(subtitle) > 30:
            raise ValidationError(f"App subtitle too long ({len(subtitle)} chars). Maximum is 30 characters.")
    ...
    def update_privacy_url(self, app_id: str, privacy_url: str, locale: str = "en-US") -> bool:
        ...
        for loc in localizations['data']:
            if loc['attributes']['locale'] == locale:
                return self.update_app_info_localization(loc['id'], {'privacyPolicyUrl': privacy_url})
    ...
    def update_app_description(self, app_id: str, description: str, locale: str = "en-US") -> bool:
        ...
        if len(description) > 4000:
            raise ValidationError(f"Description too long ({len(description)} chars). Maximum is 4000 characters.")
        
        # Get editable version
        version = self.get_editable_version(app_id)
        if not version:
            raise ValidationError(f"No editable version found for app {app_id}. "
        ...
        for loc in localizations['data']:
            if loc['attributes']['locale'] == locale:
                return self.update_app_store_version_localization(loc['id'], {'description': description})
    ...
    def update_app_keywords(self, app_id: str, keywords: str, locale: str = "en-US") -> bool:
        ...
        if len(keywords) > 100:
            raise ValidationError(f"Keywords too long ({len(keywords)} chars). Maximum is 100 characters.")
    ...
    def update_promotional_text(self, app_id: str, promo_text: str, locale: str = "en-US") -> bool:
        ...
        if len(promo_text) > 170:
            raise ValidationError(f"Promotional text too long ({len(promo_text)} chars). Maximum is 170 characters.")
        ...
        for loc in localizations['data']:
            if loc['attributes']['locale'] == locale:
                return self.update_app_store_version_localization(loc['id'], {'promotionalText': promo_text})
