...
class ReportProcessor:
    ...
    def get_sales_summary(
        ...
        if 'Apple Identifier' in sales_df.columns:
            ...
            for app_id, group in app_groups:
                ...
                by_app[str(app_id)] = {
                    ...
                    'revenue': float(group['Developer Proceeds'].sum()) if 'Developer Proceeds' in group.columns else 0.0,
                    'countries': group['Country Code'].nunique() if 'Country Code' in group.columns else 0
                }
        ...
        if 'Country Code' in sales_df.columns:
            ...
            for country, group in country_groups:
                by_country[country] = {
                    'units': int(group['Units'].sum()),
                    'revenue': float(group['Developer Proceeds'].sum()) if 'Developer Proceeds' in group.columns else 0.0,
                    'apps': group['Apple Identifier'].nunique() if 'Apple Identifier' in group.columns else 0
                }
        ...
        if by_country:
            ...
            top_countries = sorted(by_country.items(), key=lambda x: x[1]['revenue'], reverse=True)[:5]
    ...
    def get_subscription_analysis(
        ...
        if not sub_df.empty and 'App Apple ID' in sub_df.columns:
            ...
            for app_id, group in app_groups:
                app_name = group['App Name'].iloc[0] if 'App Name' in group.columns else f"App {app_id}"
                analysis['by_app'][str(app_id)] = {
                    'name': app_name,
                    'active_subscriptions': int(group['Active Subscriptions'].sum()) if 'Active Subscriptions' in group.columns else 0,
                    'total_revenue': float(group['Proceeds'].sum()) if 'Proceeds' in group.columns else 0.0
                }
    ...
    def _analyze_subscription_events(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'Event' in df.columns:
            ...
            if 'Subscribe' in event_counts and 'Cancel' in event_counts:
                metrics['cancellation_rate'] = float(event_counts['Cancel'] / event_counts['Subscribe'])
    ...
    def compare_periods(
        ...
        comparison_summary = self.get_sales_summary(start_date=comparison_start, end_date=comparison_end)
